...
def select_best_measurement(current_measurements, calibrated_measurements):
    """
    Select the measurement thats highest % to calibration value.  that's closest to its calibrated value (most stable)
                ...
                for hand_idx, (hand_landmarks, handedness) in enumerate(zip(results.multi_hand_landmarks, results.multi_handedness)):
                    ...
                    # -------------------------------
                        ...
                        if hand_distance is not None:
                            ...
                            # Maintain histories if not exists
                            # Maintain histories if not exist
                            if 'finger_world_history' not in globals():
                                finger_world_history = {tip_idx: [] for tip_idx in FINGER_TIPS}
                            ...
                            for tip_idx in FINGER_TIPS:
                                ...
                                real_x, real_y, real_z, _ = calculate_fingertip_3d_position(
                                    lm, tip_idx, hand_landmarks.landmark,
                                    hand_distance, frame_width, frame_height, focal_length_estimate
                                )

                                if real_z is None:
                                    continue

                                # Always append to history for velocity
                                hist = finger_world_history[tip_idx]
                                hist.append(real_y)
                                if len(hist) > 5:
                                    hist.pop(0)

                                # Compute downward velocity
                                velocity = 0
                                if len(hist) >= 2:
                                    velocity = hist[-2] - hist[-1]  # positive = downward

                                # Only consider fingers below threshold plane
                                if real_y >= Y_THRESHOLD and velocity > 1.0:
                                    active_fingers.append((tip_idx, velocity, real_x, real_y, real_z, x_px, y_px))

                            # Only process if exactly one finger is active
                            if len(active_fingers) == 1:
                                tip_idx, velocity, real_x, real_y, real_z, x_px, y_px = active_fingers[0]

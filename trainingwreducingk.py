...
class TrainingConfig:
    ...
    initial_k: int = 5  # Starting k value for top_k_categorical_accuracy
    k_threshold: float = 0.2  # Threshold to decrease k
    min_k: int = 1  # Minimum k value
...
class DynamicKCallback(Callback):
    """Custom callback to dynamically adjust k value in top_k_categorical_accuracy"""
    
    def __init__(self, config: TrainingConfig, model_dir: Path):
        super().__init__()
        self.config = config
        self.current_k = config.initial_k
        self.k_threshold = config.k_threshold
        self.min_k = config.min_k
        self.model_dir = model_dir
        self.k_history = []
        self.best_val_top_k = 0.0
        
        # Create new metric instance with initial k
        self.top_k_metric = TopKCategoricalAccuracy(k=self.current_k, name=f'top_{self.current_k}_categorical_accuracy')
        
    def on_train_begin(self, logs=None):
        """Initialize at training start"""
        logger.info(f"Dynamic K Callback initialized with k={self.current_k}")
        self.k_history.append(self.current_k)
    
    def on_epoch_end(self, epoch, logs=None):
        """Check and update k value at end of each epoch"""
        if logs is None:
            return
            
        # Get current validation top-k accuracy
        val_top_k_key = f'val_top_{self.current_k}_categorical_accuracy'
        val_top_k = logs.get(val_top_k_key, 0.0)
        
        # Update best validation top-k
        if val_top_k > self.best_val_top_k:
            self.best_val_top_k = val_top_k
        
        # Check if we should decrease k
        if val_top_k >= self.k_threshold and self.current_k > self.min_k:
            old_k = self.current_k
            self.current_k -= 1
            
            logger.info(f"Epoch {epoch + 1}: Validation top-{old_k} accuracy reached {val_top_k:.4f} "
                       f"(>= {self.k_threshold}). Decreasing k from {old_k} to {self.current_k}")
            
            # Update the model's metric
            self._update_model_metric()

            
        self.k_history.append(self.current_k)
        
        # Log current k value and accuracy
        logger.info(f"Epoch {epoch + 1}: Current k={self.current_k}, "
                   f"Val top-{self.current_k} accuracy: {val_top_k:.4f}")
    
    def _update_model_metric(self):
        """Update the model's top-k metric with new k value"""
        # Find and update the top-k metric in the model
        logger.info("try update metric")
        for i, metric in enumerate(self.model.metrics):
            logger.info(f"metric num {i}, {metric}")
            if metric.name == "compile_metrics":
                print(metric)
                for j, inmetric in enumerate(metric.metrics):
                    if isinstance(inmetric, TopKCategoricalAccuracy):
                        
                        # Create new metric with updated k
                        new_metric = TopKCategoricalAccuracy(k=self.current_k, 
                                                        name=f'top_{self.current_k}_categorical_accuracy')
                        logger.info("updated metric")
                        metric.metrics[j] = new_metric
                        print(new_metric)
                        print(metric)
                        print(metric.metrics)
                        self.model.metrics = new_metric
                        
                        break
    
    def on_train_end(self, logs=None):
        """Save k history at end of training"""
        k_data = {
            'k_history': self.k_history,
            'final_k': self.current_k,
            'initial_k': self.config.initial_k,
            'threshold': self.k_threshold,
            'best_val_top_k': float(self.best_val_top_k)
        }
        
        with open(self.model_dir / "dynamic_k_history.json", 'w') as f:
            json.dump(k_data, f, indent=2)
        
        logger.info(f"Training completed. Final k value: {self.current_k}")
        logger.info(f"Best validation top-k accuracy: {self.best_val_top_k:.4f}")
...
class ModelBuilder:
    ...
    def create_model(input_shape: Tuple[int, int], num_classes: int, config: TrainingConfig) -> tf.keras.Model:
        """Create an improved gesture recognition model with dynamic top-k metric"""
        ...
        # Compile with advanced optimizer settings and dynamic top-k metric
        optimizer = Adam(
        ...
        # Start with initial k value
        initial_top_k_metric = TopKCategoricalAccuracy(k=config.initial_k, 
                                                      name=f'top_{config.initial_k}_categorical_accuracy')
        
        model.compile(
            optimizer=optimizer,
            loss='categorical_crossentropy',
            metrics=['accuracy', initial_top_k_metric]
        )
        
        logger.info(f"Model created with {model.count_params():,} parameters")
        logger.info(f"Initial top-k metric: k={config.initial_k}")
        model.summary(print_fn=logger.info)
        
        return model

class GestureTrainer:
    """Main training class with enhanced features and dynamic k"""
    
    def __init__(self, config: TrainingConfig):
        self.config = config
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.model_dir = Path(f"models/{self.timestamp}")
        self.model_dir.mkdir(parents=True, exist_ok=True)
        
    def setup_callbacks(self, model: tf.keras.Model) -> List[tf.keras.callbacks.Callback]:
        """Setup training callbacks including dynamic k callback"""
        callbacks = [
            EarlyStopping(
            ...
            # Add dynamic k callback
            DynamicKCallback(self.config, self.model_dir)
        ]
        return callbacks
    def train(self) -> Tuple[tf.keras.Model, GestureDataLoader]:
        """Enhanced training pipeline with dynamic k metric"""
        logger.info("=" * 50)
        logger.info("Starting Enhanced Gesture Recognition Training with Dynamic K")
        logger.info("=" * 50)
        ...
        # Setup callbacks (including dynamic k)
        callbacks = self.setup_callbacks(model)
        
        # Train model
        logger.info(f"Training model for up to {self.config.epochs} epochs...")
        logger.info(f"Dynamic k will start at {self.config.initial_k} and decrease when reaching {self.config.k_threshold}")
